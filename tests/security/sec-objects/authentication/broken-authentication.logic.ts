...
class BrokenAuthenticationProbe {
  // Try whichever login path is available so the negative auth tests can run consistently.
  private async performUiLogin(page: any, email: string, password: string): Promise<{ ok: boolean; reason: string }> {
  ...
  async checkWeakPasswords(request: any, testInfo: any): Promise<void> {
    ...
    for (const weakPwd of weakPasswords) {
      ...
      // Probe registration with known-bad passwords and record any accepted values.
      try {
  ...
  async checkDefaultCredentials(request: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    const defaultCreds = [
    ...
    // Check a small set of common default credentials before moving on to stronger probes.
  ...
  async checkPasswordResetVerification(request: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    const user = await ensureTestUser(request as any);

    if (!user.email) {
      reporter.reportSkip('Password reset verification test could not run because no test user is configured.');
      return;
    }

    // Attempt the reset endpoint without proof of identity to confirm verification is required.
    try {
  ...
  async checkEnumerationProtection(request: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    const realUser = await ensureTestUser(request as any);
    const fakeEmail = 'nonexistent-' + Date.now() + '@example.com';

    if (!realUser.email) {
      reporter.reportSkip('Enumeration protection test could not run because no real test user is configured.');
      return;
    }

    // Compare a known user and a fake user to see whether the responses leak account existence.
  ...
  async checkDirectDashboardAccess(page: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    // Clear any existing authentication before trying a direct protected-route visit.
  ...
  async checkJwtManipulation(page: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    const user = await ensureTestUser(page.request as any);
    if (!user.email || !user.password) {
      reporter.reportSkip('JWT manipulation test could not run because no valid user credentials are configured.');
      return;
    }

    // Start from a valid login so token tampering can be tested in place.
    const loginAttempt = await this.performUiLogin(page, user.email, user.password);
    if (!loginAttempt.ok) {
      reporter.reportWarning(
    ...
    // Look for a JWT-like value in localStorage before attempting tampering.
    ...
    if (token) {
      // Replace the stored token with a modified value and see whether access is revoked.
  ...
  async checkDashboardXss(page: any, testInfo: any): Promise<void> {
    const reporter = new SecurityReporter(testInfo);
    const user = await ensureTestUser(page.request as any);
    if (!user.email || !user.password) {
      reporter.reportSkip('XSS dashboard sanitization test could not run because no valid user credentials are configured.');
      return;
    }

    // Log in first so the dashboard render path includes authenticated user data.
    const loginAttempt = await this.performUiLogin(page, user.email, user.password);
    if (!loginAttempt.ok) {
      reporter.reportWarning(
    ...
    // Scan the rendered dashboard HTML for obvious unsanitized script patterns.
    ...
    // Try to store an XSS payload through the profile update endpoint, if available.
    try {
...
export { BrokenAuthenticationProbe };
